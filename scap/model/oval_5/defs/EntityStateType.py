...
class EntityStateType(Model):
    ...
    def from_xml(self, parent, sub_el):
        ...
        if sub_el.text is not None:
            if sub_el.text == '':
                ...
                self.text = EntityStateType.DATATYPE_CLASS_MAPPING[self.datatype]().parse_value(sub_el.text)
    ...
    def to_xml(self):
        if self.datatype in EntityStateType.DATATYPE_CLASS_MAPPING:
            self.text = EntityStateType.DATATYPE_CLASS_MAPPING[self.datatype]().produce_value(sub_el.text)

        return super(EntityStateType, self).to_xml()
