...
from scap.collector.oval_5.Collector import Collector
...
class FamilyCollector(Collector):
    def collect(self):
        ...
        for cpe in self.host.facts['cpe']['os']:
            ...
            if CPE(part='o', vendor='linux').matches(cpe):
                item.family = EntityItemFamilyType(value='linux')
                return [item]
            elif CPE(part='o', vendor='microsoft').matches(cpe):
                item.family = EntityItemFamilyType(value='windows')
                return [item]
