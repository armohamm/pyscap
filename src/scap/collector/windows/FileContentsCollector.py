...
class FileContentsCollector(Collector):
    ...
    def collect(self):
        ...
        cmd = "$b = [System.IO.File]::ReadAllBytes('" + path +"')"
        cmd = cmd + '; ' + "[System.Convert]::ToBase64String($b)"
        return_code, out_lines, err_lines = self.host.exec_command('powershell -Command \"' + cmd + '\"')
        if len(out_lines) == 0 or return_code != 0:
            raise FileNotFoundError('Unable to collect contents of ' + self.args['path'] + ': ' + str(return_code))
