...
<cdf:Benchmark
    ...
    <cdf:status date="2007-10-09">draft</cdf:status>
    <cdf:title>XCCDF Sample for Cisco IOS</cdf:title>
    <cdf:description>
        This document defines a small set of rules for securing Cisco IOS routers. The set of rules constitute a
        <htm:i>benchmark</htm:i>. A benchmark usually represents an industry consensus of best practices. It lists steps to be taken as well as rationale for them. This example benchmark is merely a small subset of the rules that would be necessary for securing an IOS router.
    </cdf:description>
    <cdf:notice id="Sample-Terms-Of-Use" xml:lang="en">
        This document may be copied and used subject to the subject to the NIST terms of use (http://www.nist.gov/public_affairs/disclaim.htm) and the NSA Legal Notices (http://www.nsa.gov/notices/notic00004.cfm?Address=/).
    </cdf:notice>
    <cdf:front-matter>
        <htm:p>
    ...
    <cdf:Value id="exec-timeout-time" type="number" operator="less than or equal">
        ...
        <cdf:description>
            The length of time, in minutes, that an interactive session should be allowed to stay idle before being terminated.
        </cdf:description>
    ...
    <cdf:Group id="mgmt-plane" selected="1" prohibitChanges="1" weight="3">
        ...
        <cdf:description>
            Services, settings, and data streams related tosetting up and examining the static configuration of the router, and the authentication and authorization of administrators/operators.
        </cdf:description>
        ...
        <cdf:Rule id="no-finger-service-base" selected="0" weight="5.0" prohibitChanges="1" hidden="1" abstract="1" cluster-id="finger">
            ...
            <cdf:description>
                Disable the finger service, it can reveal information about logged in users to unauthorized parties. (For
                <cdf:sub idref="os-name"/>
            ...
            <cdf:impact-metric>AV:R/AC:L/Au:N/C:P/I:N/A:N</cdf:impact-metric>
            <cdf:fixtext fixref="no-finger" xml:lang="en">
                Turn off the finger service altogether, it is
                <htm:i>very</htm:i>
                rarely used.
            </cdf:fixtext>
        ...
        <cdf:Rule id="req-exec-timeout" selected="1" weight="8" multiple="1">
            ...
            <cdf:fix strategy="configure" disruption="low" system="urn:xccdf:fix:commands">
                line vty 0 4 exec-timeout
                <cdf:sub idref="exec-timeout-time"/>
            </cdf:fix>
    ...
    <cdf:Group id="ctrl-plane" selected="1" prohibitChanges="1" weight="3">
        ...
        <cdf:description>
            Services, settings, and data streams that support the operation and dynamic status of the router.
        </cdf:description>
        ...
        <cdf:Rule id="enabled-buffered-logging-at-level" selected="1" prohibitChanges="0" weight="8.5">
            ...
            <cdf:description>
                Make sure that buffered logging is enabled, and that the buffered logging level to one of the appropriate levels, Warning or higher.
            </cdf:description>
            <cdf:question>Check buffered logging and level</cdf:question>
            <cdf:fix>
                logging on logging buffered
                <cdf:sub idref="buffered-logging-level"/>
            </cdf:fix>
    ...
    <cdf:Group id="data-plane" selected="1" prohibitChanges="1" weight="2">
        ...
        <cdf:description>
            Services and settings related to the data passing through the router (as opposed to directed to it). Basically, the data plane is for everything not in control or mgmt planes.
        </cdf:description>
        ...
        <cdf:Group id="routing-rules" selected="1" prohibitChanges="1">
            ...
            <cdf:Rule id="no-directed-broadcast" weight="7" multiple="1" selected="1" prohibitChanges="1">
                ...
                <cdf:fix>
                    interface
                    <cdf:instance context="interface"/>
    ...
    <cdf:TestResult id="ios-test-5" end-time="2004-09-25T13:45:02-04:00">
        ...
        <cdf:identity authenticated="1" privileged="1">admin_bob</cdf:identity>
        ...
        <cdf:target-facts>
            ...
            <cdf:fact type="string" name="urn:scap:fact:asset:identifier:host_name">
                lower
            </cdf:fact>
            <cdf:fact type="string" name="urn:scap:fact:asset:identifier:ipv4">
                192.168.248.1
            </cdf:fact>
            <cdf:fact type="string" name="urn:scap:fact:asset:identifier:ipv6">
                2001:8::1
            </cdf:fact>
        </cdf:target-facts>
        <cdf:set-value idref="exec-timeout-time">10</cdf:set-value>
