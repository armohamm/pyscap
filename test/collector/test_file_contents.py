...
def test_exists():
    path = str(pathlib.Path(str(pytest.config.rootdir)) / 'test' / 'scap' / 'model' / 'test_xlink.xml')
...
def test_not_exists():
    c = host.load_collector('FileContentsCollector', {'path': str(pathlib.Path(str(pytest.config.rootdir)) / 'test' / 'scap' / 'model' / 'nope.xml')})
    with pytest.raises(FileNotFoundError):
        c.collect()

def test_args():
    with pytest.raises(ArgumentException):
        host.load_collector('FileContentsCollector', {})

# TODO actual binary file read
