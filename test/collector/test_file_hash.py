...
def test_hash(oval_family, hash_type, hash_value):
    ...
    c = host.load_collector('FileHashCollector', {'type': hash_type, 'path': str(pathlib.Path(str(pytest.config.rootdir)) / 'test' / 'scap' / 'model' / 'test_xlink.xml')})
    assert c.collect() == hash_value

def test_not_exists():
    c = host.load_collector('FileHashCollector', {'type': 'MD5', 'path': str(pathlib.Path(str(pytest.config.rootdir)) / 'test' / 'scap' / 'model' / 'nope.xml')})
